...
class Login:
    def __init__(self, root):
        ...
        self.root.geometry("1199x680+100+58")
        self.root.resizable(False, False)

        # Background Image
        self.bg=ImageTk.PhotoImage(file="Images/123.jpg")
        ...
        submit = Button(Frame_login, command=self.check_function, cursor= "hand2", text="Login?", bd=0, font=("Goudy old style", 15), bg="#6162FF", fg="white").place(x=90, y=320)

    def check_function(self):
            if self.username.get() == "" or self.password.get() == "":
                messagebox.showerror("Error", "All fields are required", parent=self.root)
            elif self.username.get() != "Goodness" or self.password.get() != "123456":
